...
**v0.2.0 测试版。** 首次实机验证采用已有 WireGuard 私网接入；全新机器安装路径尚未完成独立端到端验收。
...
- 网页文件管理：SFTP 目录浏览、上传、下载、新建目录、重命名和删除。
- Windows 当前登录桌面的画面、鼠标和常用键盘控制；Linux 节点不显示桌面入口。
- 三种远程操作优先 WebRTC 直连，约 12 秒未连通则使用现有 HTTPS/WebSocket 中转，界面显示当前路径。
...
“组网设备”提供 SSH、文件、桌面按钮。网页与连接协商经过服务器；WebRTC 直连成功后，业务数据在浏览器与目标设备之间传输。直连不通时使用现有 TCP 18443 的 WebSocket 中转，不需要另装 TURN 或开放新的服务器端口。STUN 默认使用 `stun.cloudflare.com:3478`。客户端排除原 WireGuard 虚拟 IP 作为 ICE 本地候选，避免将经中心服务器的隧道路由标成直连。

Windows 需要升级客户端和后台服务；SSH/SFTP 通过本机 OpenSSH 登录安装时指定的 Windows 用户，复用该用户的文件权限。安装器自动安装 OpenSSH Server、设为自启动、生成本机专用密钥并追加授权，入网后通过已验证的 HTTPS 连接获取并追加服务端公钥。既有授权密钥和 sshd 配置保留。防火墙默认允许 `10.77.0.0/24` 的 SSH；自定义网段请传 `-PrivateNetwork`。首次安装需要能访问 Windows 功能更新源。自定义 SSH 端口或非标准 AuthorizedKeysFile 会明确停止自动配置，需人工核对。

文件传输按块处理，网页单文件上限 128 MB；上传不覆盖同名文件，失败可能留下部分文件，可确认后删除重传；目录删除仅支持空目录。远程桌面是轻量主屏幕 JPEG 帧模式，目标电脑必须已有用户登录且托盘程序正在运行；锁屏、UAC 安全桌面和高权限窗口不绕过，不提供音频、剪贴板、独立 RDP 会话或完整系统快捷键。原生键盘输入使用目标电脑输入法。桌面查看期间托盘提示管理员正在查看。

直连使用 WebRTC DTLS 加密；回退中转使用 HTTPS/WebSocket，两端 TLS 在可信服务器终止，服务器可接触中转内容。每个远程会话最长 30 分钟；会话打开/关闭写入事件日志，管理会话失效后在下一次信令心跳关闭。Linux 服务端的 SSH/SFTP 仍校验已配置的主机指纹。

服务器本机程序可通过可选的 `localProxyListen` 使用网页中“公网服务器”的出口策略。例如在服务器配置增加 `"localProxyListen": "127.0.0.1:17890"` 后重启服务。该入口兼容 HTTP 代理与 SOCKS5 CONNECT（TCP），仅允许绑定回环地址，无需本机程序提供设备令牌；服务器上的本地用户均可使用。`HTTP_PROXY` / `HTTPS_PROXY` 可设为 `http://127.0.0.1:17890`，`ALL_PROXY` 可设为 `socks5h://127.0.0.1:17890`。不提供 SOCKS5 UDP 转发，也不继承 Mihomo 的规则分流；出口及失败行为由网页策略决定。迁移旧混合代理时可沿用其地址，先测试再停止旧服务，避免端口冲突。
